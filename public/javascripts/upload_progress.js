var UploadProgress = {
  ...
  monitor: function(upid) {
    if(!this.periodicExecuter) {
      this.periodicExecuter = new PeriodicalExecuter(function() {
        ...
        new Ajax.Request('/upload/progress?upload_id=' + upid);
      }, 3);
    }
  ...
  update: function(total, current) {
    ...
    var statusHTML = status.toPercentage();
		$('results').innerHTML   = current.toHumanSize() + ' of ' + total.toHumanSize() + " uploaded.</small>";
    this.StatusBar.update(status, statusHTML);
  },
  
  finish: function(order_id, upload_type) {
    this.uploading = false;
    this.StatusBar.finish();
    $('results').innerHTML = 'Upload finished!';
		new Ajax.Request(('/upload/finished/' + order_id + '?type=' + upload_type), {asynchronous:true});
  },
