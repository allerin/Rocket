    ...
    class AuthorizedNetGateway < Gateway
      ...
      def add_address(post, options)      
        ...
        if address = options[:billing_address] || options[:address]
          post[:address]    = address[:address1]    if address.has_key?(:address1)
          post[:company]    = address[:company]     if address.has_key?(:company)
          post[:city]       = address[:city]        if address.has_key?(:city)
          post[:state]      = address[:state]       if address.has_key?(:state)
          post[:country]    = address[:country]     if address.has_key?(:country)
          post[:phone]      = address[:phone]       if address.has_key?(:phone)
          post[:zip]        = address[:zip]         if address.has_key?(:zip)          
        end        
      end
