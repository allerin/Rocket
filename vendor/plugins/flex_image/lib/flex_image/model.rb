module FlexImage
  ...
  class Model < ActiveRecord::Base
    ...
    # By default the master images are stored as PNG images.  This allows the master image to be stored in a lossless format.
    # The image is then manipulated and compress on the way to the client.  If these master images are taking up too much
    # space, you can store them as jpegs instead by setting this class attribute to <tt>:jpg</tt>.  <tt>:png</tt> is the 
    # default.
    # 
    #   self.storage_format = :jpg
    #   
    def self.storage_format=(value)
    ...
    def data=(file)
      ...
      if (file.respond_to?(:size) and file.size > 0) or (file.kind_of?(File))
        ...
        begin
          ...
          img.format = @@storage_format.to_s.upcase
    ...
    def to_jpg!(quality = nil)
      ...
      rmagick_image.colorspace = Magick::RGBColorspace
#      rmagick_image.strip!
      save_image(quality)
    end
    ...
    private
      ...
      def delete_image_from_file_store
        File.delete(file_path) if @@file_store && File.exists?(file_path)
      end
